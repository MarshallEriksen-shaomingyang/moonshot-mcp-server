...
def find_py_files(base_dir):
    ...
    try:
        ...
        if not py_files:
            raise Exception(f"在 {base_dir} 中未找到任何 .py 文件")
        else:
            print(f"找到 {len(py_files)} 个 .py 文件")
            return py_files
    except Exception as e:
        raise Exception(f"查找 .py 文件时出错: {e}") from e


def extract_messages(py_files, output_dir, domain="messages"):
    """使用 xgettext 提取翻译字符串"""
    if not py_files:
        raise Exception("没有 .py 文件可供提取")

    try:
        os.makedirs(output_dir, exist_ok=True)
        pot_file = os.path.join(output_dir, f"{domain}.pot")
        ...
        print(f"已生成 POT 文件: {pot_file}")
        return pot_file
    except subprocess.CalledProcessError as e:
        raise Exception(f"xgettext 执行失败: {e.stderr}") from e
    except Exception as e:
        raise Exception(f"提取翻译字符串时出错: {e}") from e


def create_po_files(pot_file, locale_dir, languages):
    """从 POT 文件创建新的 PO 文件
    ...
    try:
        for lang in languages:
            ...
            if not os.path.exists(po_file):
                ...
                print(f"已为语言 {lang} 创建新的 PO 文件: {po_file}")
            else:
                print(f"PO 文件已存在: {po_file}")
    except subprocess.CalledProcessError as e:
        # 使用 errors='replace' 来处理无法解码的字符
        stderr = e.stderr.decode("utf-8", errors="replace")
        raise Exception(f"msginit 执行失败: {stderr}") from e
    except Exception as e:
        raise Exception(f"创建 PO 文件时出错: {e}") from e


def merge_po_files(pot_file, locale_dir):
    ...
    try:
        ...
        if not po_files:
            print(f"在 {locale_dir} 中未找到 .po 文件，跳过合并")
            return
        ...
        for po_file_path in po_files:
            if po_file_path.exists():
                ...
                print(f"正在合并现有 PO 文件: {po_file_path}")
            else:
                # 如果 .po 文件不存在，创建一个新的
                po = polib.POFile()
                po.metadata = pot.metadata.copy()  # 复制 .pot 的元信息
                print(f"创建新的 PO 文件: {po_file_path}")
            ...
            print(f"已更新 PO 文件: {po_file_path}")

    except Exception as e:
        raise Exception(f"合并 .po 文件时出错: {e}") from e


def compile_po_files(locale_dir):
    """编译所有 .po 文件为 .mo 文件"""
    try:
        locale_path = Path(locale_dir)
        if not locale_path.exists():
            raise Exception(f"locale 目录 {locale_dir} 不存在，跳过编译")

        po_files = list(locale_path.rglob("*.po"))
        if not po_files:
            raise Exception(f"在 {locale_dir} 中未找到 .po 文件")
        ...
        for po_file in po_files:
            ...
            subprocess.run(cmd, check=True, capture_output=True, text=True)
            print(f"已生成 MO 文件: {mo_file}")
    except subprocess.CalledProcessError as e:
        raise Exception(f"msgfmt 执行失败: {e.stderr}") from e
    except Exception as e:
        raise Exception(f"编译 .po 文件时出错: {e}") from e
...
def main():
    ...
    parser.add_argument(
        "--base-dir",
        default=os.getenv("MOONSHOT_SRC_DIR", os.path.dirname(os.path.dirname(__file__))),
